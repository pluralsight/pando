...
describe('getFlexProps', () => {
  ...
  test('should apply the corresponding gap type based on direction', () => {
    expect(getFlexProps({ direction: 'column' })?.style.rowGap).toEqual('1rem')
    expect(
      getFlexProps({ direction: reverseColumnProp })?.style.rowGap
    ).toEqual('1rem')
    expect(getFlexProps({ direction: 'row' })?.style.columnGap).toEqual('1rem')
    expect(
  ...
  test('should accept a direction', () => {
    expect(getFlexProps({ direction: 'column' })?.style.flexDirection).toEqual(
      'column'
    )
    expect(
