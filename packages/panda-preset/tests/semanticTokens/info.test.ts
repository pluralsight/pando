...
describe('infoTokens', () => {
  ...
  test('should have a text 300 value', () => {
    expect(infoTokens.info.text[300].value._inkyBlueTheme.base).toEqual(
      '#D5F0FF',
    )
    expect(infoTokens.info.text[300].value._inkyBlueTheme._lightMode).toEqual(
      '#003655',
    )
    expect(infoTokens.info.text[300].value._inkyBlueTheme._darkMode).toEqual(
      '#D5F0FF',
    )
  })
})
